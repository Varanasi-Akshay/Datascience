...
class NeuralNetMLP(object):
    ...
    def _compute_cost(self, y_enc, output):
        ...
        """
        L2_term = (
            self.l2 * (np.sum(self.w_h ** 2.) + np.sum(self.w_out ** 2.))
        )
    ...
    def fit(self, X_train, y_train, X_valid, y_valid):
        ...
        for i in range(self.epochs):
            ...
            train_acc = (
                (np.sum(y_train == y_train_pred)).astype(np.float) / X_train.shape[0]
            )
            valid_acc = (
                (np.sum(y_valid == y_valid_pred)).astype(np.float) / X_valid.shape[0]
            )

            sys.stderr.write(f'\rEpoch {i+1}/{self.epochs} | Cost: {cost:.2f} '
