...
Normally, we would have a different row for each of the 192 subjects that had no heart disease 
and were at level 4 snoring.  
...
For a linear probability model, we are modeling the mean itself and not a transformation, 
thus we use the identity function. 
The model is pie = a + ß X.
...
freq count ;
model heartdisease =snoring/dist=bern link=identity;                                
run;
...
as the link function.  That is, we are modeling the log(odds) = a + ß X. 
