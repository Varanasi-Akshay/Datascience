...
This gives the difefrent Y*X plots for each level of Z (on the same figure)*/

symbol1 v=circle i = join l=32  c = red;
symbol2 i = join v=star l=32  c = blue;
PROC GPLOT DATA=results_int;
PLOT  predicted_bp*age=gender/ OVERLAY;
RUN;
...
and at best do not give the full information.   
...
ALso, the results could be misleading like to say there is no difference between the genders.
...
reparametrize age by using age2=age-33.  Note, we need to recreate our
age variable*/ 
...
symbol1 v=circle i = join l=32  c = red;
symbol2 i = join v=star l=32  c = blue;
PROC GPLOT DATA=results_int2;
PLOT  predicted_bp2*age2=gender/ OVERLAY;
RUN;

/* Looking at the two plots, you can see that when age=0 corresponds to age2=-13.
Also, we have that when age2=0, this corresponds to age=33. */
